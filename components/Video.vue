<template>
  <div class="container-loc animate__animated animate__fadeIn">
    <v-row class="ma-0 pa-0" style="width: 100%">
      <v-column v-if="!isFetched" class="vid-con">
        <v-skeleton-loader
          ...
          <v-row class="d-flex flex-column mb-4" no-gutters>
            ...
            <v-row class="d-flex ma-0 mt-1 align-center">
              <span class="d-flex align-center"
                ><v-icon>mdi-eye</v-icon> 11,672 Views</span
              >
              <v-spacer></v-spacer>
              <v-btn
                icon
              ...
              <h4 class="justify-center" v-if="post.likes">
                {{ post.likes.length }}
              </h4>
            </v-row>
          </v-row>
      ...
      <v-column class="d-flex align-center ml-2">
        ...
        <v-row class="d-flex flex-column px-7" no-gutters>
          ...
          <small v-if="post.comments"
            >{{ post.comments.length }} comments and counting...</small
          >
        </v-row>
        ...
        <v-row class="ma-0 px-7">
          <div v-if="post.comments">
            ...
            >
              <v-list-item-avatar>
                <v-img
                  src="https://www.biography.com/.image/ar_1:1%2Cc_fill%2Ccs_srgb%2Cfl_progressive%2Cq_auto:good%2Cw_1200/MTQ3NTI2OTA4NzY5MjE2MTI4/drake_photo_by_prince_williams_wireimage_getty_479503454.jpg"
                ></v-img>
              </v-list-item-avatar>
              <v-list-item-content>
                <v-list-item-subtitle class="flex-wrap"
                  >{{ comment.name }} -
                  {{ convertDate(comment.createdAt) }}</v-list-item-subtitle
                >
...
export default {
  async fetch() {
    ...
    for (let x = 0; x < this.posts.length; x++) {
      var currComments = await this.$store.dispatch(
        "api/listComments",
        this.posts[x].id
      );
      this.posts[x].comments = currComments;
      if (this.posts[x].likes == true) {
  ...
  methods: {
    ...
    async addComment(post) {
      ...
      try {
        var newComment = await this.$store.dispatch(
          "api/createComment",
          comment
        );
    ...
    async like(post) {
      if (this.likedPosts.includes(post)) {
        ...
        for (let i = 0; i < this.posts.length; i++) {
          if (this.posts[i].id == post) {
            this.posts[i].likes = this.posts[i].likes.filter(
              (item) => item !== this.currId
            );
          }
        }
